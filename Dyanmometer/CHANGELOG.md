...
## [V2.121 beta / v2.10.79] - 2026-09-15

### 徹底剷除虛構之「uF.05」幽靈暫存器讀取、回歸 KEB COMBIVERT F5 原廠規範 (銘牌 dr.05 與基頻 uF.00)、並修復同步檢查或條件誤報正常之缺陷

### 現象與佐證
- **使用者回報質疑**：使用者在操作堵轉頁面實測反饋：
  `為何現在顯示uf.05=0.00Hz？然後在同步檢查時還告訴我正常？為何要觀察uf.05？原因是甚麼？`
- **實測數據佐證**：
  - 介面常駐橫條顯示：`實測頻率: dr.05=51.50 Hz | uf.05=0.00 Hz | uf.00=51.50 Hz (已同步)`
  - 點擊「[同步檢查]」彈出對話框：`【[O] KEB 參數蒐集與同步驗證正常】`

### 致命根因 (Root Cause)
1. **虛構的幽靈暫存器讀取**：
   - 在 KEB COMBIVERT F5 原廠手冊與通訊規範中，V/f 特性參數群組（`uF`）的基準額定頻率為 **`uF.00` (地址 0x0500)**，馬達銘牌額定頻率為 **`dr.05` (地址 0x0605)**；
   - 原廠手冊中根本沒有任何所謂「拐點/額定頻率 `uf.05` (0x0505)」的參數；
   - 先前版本盲目向變頻器讀取 `0x0505`，變頻器回傳 null/0，程式未防禦直接賦予 0，致使常駐橫條顯示 `uf.05=0.00 Hz`；
2. **同步檢查判斷式條件過於寬鬆**：
   - `CheckAndCollectDrUfParams` 中的頻率同步判定式：
     `bool freqSynced = (drFreq > 0 && ((uf05Freq > 0 && 吻合) || (uf00Freq > 0 && Math.Abs(drFreq - uf00Freq) <= 0.5)));`
   - 因採用 `||` 寬鬆邏輯，現場實測 `uF.00 = 51.50 Hz` 與 `dr.05 = 51.50 Hz` 完全吻合，條件成立判為 `freqSynced = true`；
   - 導致系統將 `uf05Freq = 0.00 Hz` 忽略不計，依然誤報「正常已同步」；
3. **觀察 uf.05 毫無物理意義**：
   - KEB 變頻器控制馬達的核心頻率指標僅有馬達銘牌額定頻率 (`dr.05`) 與變頻器 V/f 特性基頻 (`uF.00`)，`uf.05` 純屬虛構，造成操作者嚴重視覺混淆與困惑。

### 精確修復方案
1. **徹底移除 `0x0505` (uf.05) 讀取與變數**：
   - 在 `Dynamometer_TestEquivCircuit.cs` 中移除向 `0x0505` 的讀取、換算與參數傳遞；
2. **回歸 KEB 原廠真實雙頻率監控**：
   - 卡片 3 Row 4 常駐橫條更新為：
     `實測頻率: dr.05=xx.xx Hz (銘牌) | uF.00=xx.xx Hz (基頻)`
   - 僅呈現真實存在的 `dr.05` (馬達銘牌) 與 `uF.00` (變頻器基頻)；
3. **嚴格同步比對檢查**：
   - 廢除 `||` 寬鬆邏輯，直接嚴格比對 `Math.Abs(drFreq - uf00Freq) <= 0.5`；
   - 兩者一致（如均為 51.50 Hz）即亮起清新綠底標示「已同步」；若存在差異立即亮警示黃底並彈窗告警；
4. **堵轉試驗工作線程回歸 `0x0500` (uF.00)**：
   - `LockedSweepWorker` 中全面回歸以 `uF.00` 作為基頻提取與備援。
...
## [V2.120 beta / v2.10.78] - 2026-09-15

### 徹底修復線上更新「HTTP 404」致命缺陷、排除 .gitignore 誤阻擋 Release 執行檔與自動化發布強制推送雙保險機制

### 現象與佐證
- **使用者回報現象**：使用者在動力計 HMI 點擊線上自動更新時，跳出錯誤提示：
  `下載或替換失敗: 伺服器回應異常: 404 (HTTP/1.1 404 Not Found)`
- **實測網路請求佐證**：
  - 查詢雲端 Firebase RTDB 之 `/update/version.json`，取得下載清單：
    `download_url`: `https://raw.githubusercontent.com/Isaacyang34/Homepage/gh-pages/Dyanmometer/Release/Dynamometer_HMI_V2.10.77_Portable/Dynamometer_HMI_Pro.exe`
  - 使用 `curl.exe -s -I` 向該 URL 發起 HTTP HEAD 請求，回傳：
    `HTTP/2 404 Not Found`

### 致命根因 (Root Cause)
1. **`.gitignore` 全域忽略規則誤殺發布執行檔**：
   - 根目錄 `.gitignore` 第 3 行設定為 `*.exe`，但未為發布目錄設置例外規則；
2. **`package_release.ps1` 暫存指令缺乏強制參數 `-f`**：
   - `package_release.ps1` 第 190 行執行：
     `& $gitExe -C $repoRoot add "Dyanmometer/Release/"`
   - 因受到 `.gitignore` 阻擋，新版本目錄 `Dynamometer_HMI_V2.10.77_Portable/Dynamometer_HMI_Pro.exe` 完全未被加入 Git 暫存區，後續 commit 與 push 到 `gh-pages` 分支時，遠端倉庫中根本不存在該 exe 檔案；
3. **雲端版本指標與真實倉庫檔案脫節**：
   - `package_release.ps1` 在未確認檔案是否成功推送到 `gh-pages` 的情況下，即將 raw.githubusercontent.com 的新版路徑寫入 Firebase RTDB，導致客戶端線上更新時請求不存在的檔案，引發 HTTP 404 例外。
...
1. **修改根目錄 `.gitignore`**：
   - 於 `*.exe` 下方新增白名單例外：
     ...
     `!Release/**/*.exe`
     永久允許發布目錄之原生便攜執行檔受 Git 版本控制。
2. **修改 `package_release.ps1` 自動打包腳本**：
   - 將第 190 行之 `git add` 加入 `-f` 強制旗標：
     `& $gitExe -C $repoRoot add -f "Dyanmometer/Release/" 2>&1 | Write-Host`
     達成雙保險機制，絕不漏推任何版本之可攜式主程式。
3. **版本遞增至 v2.10.78 並全面重新發布**：
...
## [V2.114 beta / v2.10.72] - 2026-09-14

### 全面建立堵轉測試「降壓硬體驗證安全閉鎖」與「瞬時突波極限跳脫」機制，徹底杜絕高壓激磁與金屬巨響衝擊

### 現象與佐證
- **使用者回報現象**：使用者實測回報：「上傳日誌，剛才有發生巨響，我猜又是控制問題，你查清楚，這個堵轉不能開玩笑的」。
- **實測日誌提取驗證**：
  提取 Firebase 雲端最新日誌 `20260914_163308.json` (`KEB_Readback_Parameters.log`)，實測數據明確記錄：
  ```
  [2026-09-14 16:32:37] 【B載台】連線成功，已讀回目前硬體配置參數快照
  - 連線通訊埠: COM2 (站號 Node: 1)
  - ud.02 (變頻器應用模式) : 1 -> F5-General (SMM)
  - cs.00 (控制架構模式)   : 0 -> V/F 開迴路 (cs00=0)
  - oP.03 (數位設定轉速)   : 1500.0 rpm (RAW: 12000)
  - Sy.50 (目前控制字狀態) : 0
  - Sy.52 (過程數據轉速)   : 1500.0 rpm (RAW: 1500)
  - dr.02 (馬達額定電壓)   : 260 V
  ```
  在 16:32:37，變頻器內部處於 `Sy.52 = 1500 rpm`（50Hz 對應同步轉速），且額定基準電壓為 `dr.02 = 260V`。

### 致命根因 (Root Cause)
1. **運轉中寫入保護導致降壓失敗**：
   在 KEB F5 變頻器中，特性曲線參數 `uF.09` (基準電壓) 具備運轉保護（Write Protected），若變頻器處於運轉狀態（Sy.50 != 0），變頻器會直接拒絕寫入 `uF.09`！
2. **缺乏降壓硬體驗證盲目給予轉速**：
   在 `Dynamometer_TestEquivCircuit.cs` 的 `LockedSweepWorker` 中，步驟 A 呼叫 `KebWriteUf09` 後，**完全未回讀驗證變頻器內部 `uf.09` 是否確實已降至安全低壓**；
   緊接著在步驟 B 直接向變頻器寫入 `Sy.52 = 1500 rpm`；
   導致變頻器在 `uF.09 = 260V` 的額定全電壓下，直接向**物理剛性鎖死**的轉子輸出 50Hz 交流電！
   馬達在 260V 全電壓短路狀態下瞬間湧入數百安培極限短路電流，變頻器與馬達定子劇烈電磁過載，爆發驚人金屬撞擊巨響！
3. **看門狗保護缺乏瞬時大電流急停**：
   原看門狗僅有 `電流 > 110% 且持續 10 秒` 的慢速保護，無法在毫秒級突波大電流發生時第一時間緊急切斷。

### 精確修復方案
1. **強制進入 nOP 降壓安全視窗 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 在寫入 `uf.09` 前，強制先下達 `Sy.50 = 0` 確保停機進入 nOP 狀態，並執行 `ClearLockedSpeedCmd()` 清空 `Sy.52` 與 `oP.03`，延遲 200ms 確保硬體處於安全靜止態。
2. **【生與死安全閉鎖】uf.09 降壓硬體驗證機制 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 寫入目標降壓值後，強制執行 5 次回讀驗證 `KebReadUf09(driveId)`；
   - **若回讀數值未確認 $\le 60\text{V}$，系統強制阻斷！嚴禁寫入任何轉速指令，嚴禁啟動變頻器！**
   - 立即彈出緊急錯誤對話框並終止試驗，徹底杜絕變頻器在額定高壓下輸出頻率的任何可能性。
3. **安全低壓激磁啟動時序 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 當且僅當硬體驗證 `uf.09 <= 60V` 成功後，才寫入測試轉速 `Sy.52`；
   - 最後由程式下達 `Sy.50 = 4` (RUN 正轉) 開始在已驗證的極低電壓下安全平穩建壓，零衝擊、零噪音。
4. **全頻率切換零衝擊保護 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 換頻前先下達 `Sy.50 = 0` 與轉速清零，再寫入新頻率電壓並驗證，再給予新轉速與啟動。
   - 試驗完成立即執行 `Sy.50 = 0` 與轉速清零。
5. **瞬時突波極限瞬間跳脫保護 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 在 `tmrLockedWatchdog_Tick` 中新增：實測電流若超過 150% 額定電流（$I_{cur} > 1.50 \times I_N$），零秒延遲立即觸發 `TriggerLockedProtectionTrip`，毫秒級切斷輸出保護硬體。
6. **發布與驗證**：
   - 升版至 `2.10.72`。
   - 通過 `csc.exe` 編譯，完成雙分支同動推送與 GitHub Release v2.10.72 發布。
...
### 徹底根除等效電路點擊「清除重測」後 1.空載、2.額定 數據未歸零之缺陷

### 現象與佐證
- **使用者回報現象**：使用者實測指出：「按下清除重測，1Noload 2額定 的數據不會被清空」。
- **問題重現還原**：
  - 使用者在等效電路分頁點擊「[X] 清除重測」按鈕並確認後，Card 1 (空載運轉數據) 的 V0、I0、P0、PF0、N0、f0 與 Card 2 (額定運轉數據) 的 TN、NN、VN、IN、PN、PFN、s 依然留存靜態數值，未呈現清空/歸零狀態；
  - 頂部狀態列依然持續顯示 `一致性: [O] 同一馬達記憶中 (1,2項數據已保留)`，造成視覺與邏輯嚴重脫節。

### 致命根因 (Root Cause)
1. **清除函式誤寫入寫死之靜態樣本數據**：
   在 `Dynamometer_TestEquivCircuit.cs` 的 `ResetEquivDataCards()` 函式中，代碼收到清除指令後，竟執行了 `numEquivV0.Value = 260m; numEquivI0.Value = 12m; numEquivP0.Value = 450m; numEquivTn.Value = 70m; numEquivNn.Value = 1465m; numEquivVn.Value = 260m; ...`，強行把舊式 SVM100S 樣本值重新填入控制項，而非將數值清空歸零（`Value = 0m`）！
2. **NumericUpDown 最小限值設限阻礙歸零**：
   Card 1 的 `numEquivF0` (測試頻率) 在初始化時設定 `Minimum = 1`，導致若嘗試設定為 `0m` 會觸發系統範圍例外，阻礙了正常的數值歸零重置。
3. **狀態列顯示邏輯未與採樣狀態聯動**：
   `RefreshEquivMotorStatus()` 僅比對馬達名稱與 dr 指紋是否一致，即便採樣已被清除（`isNoLoadDataReady = false`），依然在同馬達分支中寫死輸出 `(1,2項數據已保留)`。
4. **INI 載入端狀態同步單向缺失**：
   `LoadEquivCircuitConfig()` 僅在 `NoLoadReady == "1"` 時設為 true，當 `NoLoadReady == "0"` 時未同步將記憶標誌與狀態文字切換回待採樣。

### 精確修復方案
1. **輸入控制項全量歸零重置 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 在 `ResetEquivDataCards()` 中，將 1.空載（V0, I0, P0, PF0, N0, f0）、2.額定（TN, NN, VN, IN, PN, PFN, s）、3.堵轉（Vk, Ik, Pk, PFk）共 17 項輸入數值全數徹底歸零清空 (`Value = 0m`)。
   - `InitDefaultEquivResultsGrid()` 同動將下方結果網格全數重設為 `--`，並清空圖解快取。
2. **開放 f0 零值輸入與卡片初始預設值純淨化 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 將 `numEquivF0` 之 Minimum 下限由 1 改為 0，預設值歸零。
   - Card 1、Card 2、Card 3 建立時的所有欄位預設值一律改為 0.0m，未採樣前絕不預填虛假樣本。
3. **狀態列動態真實反映採樣狀態 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 在 `RefreshEquivMotorStatus()` 中增加判定：若 1/2/3 項數據均未採樣，狀態列明確顯示 `一致性: [--] 數據已清空，待採樣 (1.空載 / 2.額定 / 3.堵轉)`；若僅部分完成則精確標註各項狀態，徹底根除誤導。
4. **INI 設定記憶雙向同動與清除閉環 (`Dynamometer_TestEquivCircuit.cs`)**：
   - `SaveEquivCircuitConfig()` 在數值為空或預設時寫入 0。
   - `LoadEquivCircuitConfig()` 支援 `Ready == "0"` 之重置，並即時更新卡片狀態標籤為 `[--] 待採樣`。
   - 點擊「清除重測」後立即同動呼叫 `SaveLayoutConfig()` 將全 0 數據保存至 INI，確保重開程式亦維持乾淨狀態。
5. **發布與驗證**：
   - 升版至 `2.10.69`。
   - 通過 `csc.exe` 編譯，完成雙分支同動推送與 GitHub Release v2.10.69 發布。
...
### 現象與痛點
- **使用者強烈指示**：
  使用者實測指出：「同步檢查還是錯啊uf.00=25.8，你要不把參數都放到LOG回去慢慢分析，感覺你搞不定阿，一直瞎猜」。
- **問題本質分析**：
  現場變頻器透過原廠通訊回傳的原始整數值 (RAW Integer) 過去未被透明化展示。若變頻器內部存值為 `1030`（$1030 \times 0.05 = 51.5\text{ Hz}$），若誤乘 $0.025$ 就會得出 $25.75 \approx 25.8\text{ Hz}$。如果不把實機回傳的原始整數與 HEX 封包完全打印出來，任何單向猜測都是盲目的。

### 精確修復方案
1. **同步檢查全量原始封包 DUMP (`Dynamometer_TestEquivCircuit.cs`)**：
   - 在 `CheckAndCollectDrUfParams` 中，讀取：
     - `uF.00` (0x0500) 在 Set 1 與 Set 0 的原始數值 (DEC 與 HEX)。
     - `dr.05` (0x0605 / 0x0405) 在 Set 0 與 Set 1 的原始數值 (DEC 與 HEX)。
   - 透過 `WriteHmiLog("KEB_RAW_DUMP", ...)` 將原始整數、HEX 與各比例（0.05, 0.025, 0.0125, 0.1）試算結果完整輸出至主畫面日誌與檔案。
   - **在彈窗（MessageBox）中直接明文列印實測 RAW 數據**，使用者點擊「同步檢查」時，對話框內將直接顯示底層真實整數（例如：`Raw=1030 (0x0406)` 或 `Raw=2060 (0x080C)`），一目了然！
2. **即時輪詢即時輸出 RAW 日誌 (`Dynamometer_KebComm.cs`)**：
   - 在 `DoHmiKebQuery1` 與 `DoHmiKebQuery2` 中，凡輪詢到 `0x0500` 與 `0x0605`，即時在 HMI 日誌輸出其 RAW DEC 與 HEX。
3. **uF.00 導入閉環參考與常規基頻先驗鎖定 (`Dynamometer_KebComm.cs`)**：
   - 重構 `ConvertKebUf00ToFrequency(int rawUf00, double refDrFreq = 0.0)`：
     - 若已知 `dr.05` 額定頻率（如 51.5 Hz），自動以誤差最小之解析度（0.05 vs 0.025）進行閉環鎖定！
     - 先驗區間判定：若 `raw * 0.05` 落在 45.0~75.0 Hz（50Hz / 51.5Hz / 60Hz 典型馬達基頻），優先鎖定 0.05 Hz 解析度。
4. **版本發布與驗證**：
   - 升版至 `2.10.68`。
   - 通過 `csc.exe` 編譯，完成雙分支同動推送與 GitHub Release v2.10.68 發布。

---

## [V2.109 beta / v2.10.67] - 2026-09-14

### 徹底根治即時綜合監控 dr05 誤讀 0x0405 顯示為 0.6Hz 之位址誤區

### 現象與佐證
- **使用者回報現象**：使用者實測指出：「在即時綜合監控dr05是0.6」。
- **實測數據還原**：
  - 使用者在變頻器內部設定馬達額定頻率：`51.5 Hz`（實際存放於 `0x0605`，值為 `515`）。
  - 使用者本地設定檔 `dynamometer_layout.ini` 依然記錄了舊版位址：`Addr=0405`。
  - 即時輪詢向變頻器讀取 `0x0405`，變頻器回傳了某個非 0 的內部參數值：`val = 6`。
  - 舊程式判定條件：`if (!val.HasValue || val.Value == 0)`，因為 $6 \ne 0$，因此**未能觸發回退至 0x0605**！
  - 程式直接將原始值 `6` 傳入換算：$6 \times 0.1 = \mathbf{0.6\text{ Hz}}$！

### 致命根因 (Root Cause)
1. **未在 INI 載入端攔截替換**：
   - 使用者本機設定檔載入時，舊的 `0405` 依然被加入 `KebMonitorItem`。
2. **查詢端探測優先順序錯誤**：
   - 舊程式以 `item.Address` 為優先，失敗或為 0 才探測 `0x0600+`。當 `0x0405` 存在無關數值（`6`）時，被誤當成有效銘牌頻率！
   - 而正確的實機 dr 參數一律在 `0x0600 ~ 0x0605`（與堵轉同步測試一致）。

### 精確修復方案
1. **INI 載入防禦性位址自動升級 (`Dynamometer_HMI_WinForms.cs`)**：
   - Line 2546 與 Line 2589：凡偵測到 `addr 在 0x0400~0x0418` 且名稱含有 `dr` / `額定` / `馬達`，一律直接在記憶體中將位址強制替換為實機位址 `0x0600 + (addr - 0x0400)`（如 `0x0405` 強制變為 `0x0605`）。
2. **即時輪詢一律優先讀取 0x0600+ (`Dynamometer_KebComm.cs`)**：
   - 在 `DoHmiKebQuery1` 與 `DoHmiKebQuery2` 中，凡遇 dr 參數，**一律優先向變頻器讀取 0x0600+ 實機位址**（`0x0605` 讀到 `515` 即直接解析為 `51.5 Hz`），徹底避免誤讀 `0x0405`。
3. **ConvertKebDr05ToFrequency 加入異常值過濾 (`Dynamometer_KebComm.cs`)**：
   - 若 raw < 20（如誤讀到無關參數 6），直接防禦回傳 0.0，避免產生 0.6 Hz 誤導。
4. **版本發布與驗證**：
   - 升版至 `2.10.67`。
   - 通過 `csc.exe` 編譯，完成雙分支同動推送與 GitHub Release v2.10.67 發布。
| V2.107 (beta) | v2.10.65 | 2026-09-14 | 徹底消除 KEB 參數解義雙軌分裂 (Single Source of Truth 收攏)：(1)現象與核心痛點：使用者質疑「在即時綜合監控看到的數據 dr.05 和 uf.00 都不正確，但在堵轉分頁做同步檢查反而 dr.05 正確了，但 uf.00 變成 206；為何都是讀取 KEB 參數會有兩種結果，程式裡面為何不是用相同的解義邏輯？」；(2)致命根因 (Root Cause)：歷史架構雙軌分裂：①【位址雙軌分裂】：即時監控選單原將 dr 群組寫為 0x0400~0x0405，而實機變頻器 dr 實際位於 0x0600~0x0605；堵轉同步測試自行加上 0x06xx 探測因而讀到 515 (51.5Hz)，但即時監控未更新位址導致讀不到數值；②【解義邏輯分裂】：即時監控依賴 item.Scale (0x0500 誤為 0.0001 算成 0.21Hz)，而堵轉測試私自用階梯判斷 (2060 >= 200 被 * 0.1 算成 206Hz)，極數辨識又另寫一套；全案缺乏統一解析核心；(3)精確修復方案：在 `Dynamometer_KebComm.cs` 建立統一之 `ConvertKebDr05ToFrequency` 與 `ConvertKebUf00ToFrequency`，並於 `DoHmiKebQuery1`、`DoHmiKebQuery2` 即時輪詢中全面導入 0x04xx/0x06xx 雙向自動相容探測；堵轉同步檢查與極數反算全面收攏調用相同函式；候選清單全面同步更新為實機 0x06xx；(4)發布與驗證：升版至 2.10.65，經 csc.exe 編譯通過，完成雙分支同動與 GitHub Release v2.10.65 發布。 |

---

## [V2.108 beta / v2.10.66] - 2026-09-14

### 根除 uF.00 額定頻率誤被 0.0125 縮放砍半顯示為 25.8Hz 之致命缺陷

### 現象與佐證
- **使用者回報現象**：使用者實測指出：「還是錯啊，uf.00變成25.8」。
- **實測數據比對**：
  - 設定額定頻率：`51.5 Hz`。
  - 變頻器回傳原始整數值：`rawUf00 = 2060`。
  - 錯誤運算：$2060 \times 0.0125 = \mathbf{25.75} \to \mathbf{25.8\text{ Hz}}$（剛好為 $51.5 \div 2$）。

### 致命根因 (Root Cause)
1. **uF.00 物理刻度誤與 ru.03 混淆**：
   - 在 KEB F5 中，只有即時運轉輸出頻率 `ru.03` (0x0203) 才會依據馬達最高轉速 Sy.52（4000 rpm vs 8000 rpm）區分 0.0125 與 0.025 解析度。
   - 而基頻暫存器 `uF.00` (0x0500) 在變頻器硬體規範中，**標準解析度一律固定為 $1\text{ unit} = 0.025\text{ Hz}$（即 $1\text{ Hz} = 40\text{ units}$）**。使用者設定 51.5 Hz，變頻器寫入的就是 $51.5 \times 40 = 2060$。
2. **driveId/node 傳遞引發條件誤判**：
   - 舊版 `ConvertKebUf00ToFrequency` 寫了 `double stdScale = (driveId == 1) ? 0.0125 : 0.025;`。
   - 在堵轉測試中呼叫時誤傳了 `node`（而現場機台站號恰好為 `1`），導致判定為 1，進而套用了 `0.0125`。
   - $2060 \times 0.0125 = 25.75$，落在有效範圍內直接被回傳，導致 HMI 顯示出 `25.8 Hz`！
3. **INI 載入清洗的殘留比例**：
   - `Dynamometer_HMI_WinForms.cs` 原在載入 list1 時將 `0x0500` 也歸入 `0.0125`，導致 A 載台即時網格若顯示 `0x0500` 亦會被算成 `25.8`。

### 精確修復方案
1. **確立 0.025 Hz 為 uF.00 主力解析度 (`Dynamometer_KebComm.cs`)**：
   - 重構 `ConvertKebUf00ToFrequency`：直接以 `raw * 0.025` 作為核心主力分支，保證 `2060 * 0.025 = 51.50 Hz` 100% 精確命中，不再因載台編號或站號誤乘 0.0125。
2. **INI 載入清洗統一 Scale (`Dynamometer_HMI_WinForms.cs`)**：
   - Line 2544 與 2586：凡偵測到 `addr == 0x0500`，不論 list1 還是 list2，Scale 一律強制校正為 `0.025`。
3. **修正等效電路測試參數調用 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 將傳給 `ConvertKebUf00ToFrequency` 的參數修正為 `driveId`。
4. **版本發布驗證**：
   - 升版至 `2.10.66`。
   - 通過 `csc.exe` 編譯，完成雙分支同動推送與 GitHub Release v2.10.66 發布。
| V2.106 (beta) | v2.10.64 | 2026-09-14 | KEB COMBIVERT F5 額定頻率 uF.00 (0x0500) 讀取顯示 0.21Hz 縮放致命缺陷根治：(1)現象與佐證：使用者回報在變頻器設定額定頻率為 51.5Hz，但在 HMI 即時監控網格與等效電路讀回卻顯示為 0.21Hz；(2)致命根因 (Root Cause)：KEB F5 速度標準化系統中，B 載台（8000 rpm / 400 Hz 範圍，待測端）頻率暫存器採用 1 unit = 0.025 Hz 儲存，51.5 Hz 儲存為 2060；舊程式在 `Dynamometer_HMI_WinForms.cs` (Line 3129) 誤將 Scale 設為 0.0001，導致 2060 * 0.0001 = 0.206 顯示為 0.21Hz，且 INI 載入與等效電路測試中均未採用正確 0.025 解析度換算；(3)精確修復方案：在 `Dynamometer_KebComm.cs` 建立 `ConvertKebUf00ToFrequency` 智能換算函式，適配 B 載台 (0.025 Hz)、A 載台 (0.0125 Hz) 與擴展高解析度模式；在 `Dynamometer_HMI_WinForms.cs` 加入 INI 載入防禦清洗、修正候選清單 Scale 為 0.025；在 `Dynamometer_TestEquivCircuit.cs` 中統一改調用該換算；(4)發布與驗證：升版至 2.10.64，經 csc.exe 編譯通過，更新 Firebase 指標並完成雙分支同動推送與 GitHub Release v2.10.64 發布。 |

---

## [V2.107 beta / v2.10.65] - 2026-09-14

### 徹底消除 KEB 參數解義雙軌分裂 (Single Source of Truth 收攏)

### 現象與佐證
- **使用者回報現象**：
  使用者深刻質疑：「在即時綜合監控看到的數據 dr.05 和 uf.00 都不正確，但在堵轉分頁做同步檢查反而 dr.05 正確了，但 uf.00 變成 206；為何都是讀取 KEB 參數會有兩種結果，這程式裡面為何不是用相同的解義邏輯？」
- **實測數據比對**：
  - 變頻器實體設定：額定頻率 `51.5 Hz`。
  - 即時綜合監控畫面：
    - `dr.05`：顯示為 `0` 或異常空白。
    - `uF.00`：顯示為 `0.21 Hz`（因 $2060 \times 0.0001 = 0.206 \to 0.21$）。
  - 堵轉同步檢查對話框：
    - `dr.05`：顯示為正確的 `51.5 Hz`。
    - `uF.00`：顯示為 `206.0 Hz`（因舊版代碼寫了 $2060 \ge 200$ 則 $\times 0.1 = 206$）。

### 致命根因 (Root Cause)
全案在長期演進中累積了嚴重的「架構雙軌分裂 (Architecture Drift)」：
1. **位址雙軌分裂 (Address Drift)**：
   - 即時綜合監控網格清單與使用者 INI，依據早期資料將 dr 馬達銘牌群組寫為 `0x0400 ~ 0x0405`。
   - 現場機台（KEB COMBIVERT F5）的 dr 銘牌參數實際物理暫存器位於 **`0x0600 ~ 0x0605`**。
   - 堵轉測試模組與極數偵測模組後來自行補上了 `0x06xx ?? 0x04xx` 雙向探測，因此讀到了正確的 `515` (51.5Hz)；但即時監控卻始終沒有同步修正位址，導致向 `0x0405` 讀取失敗顯示為 0。
2. **解義換算邏輯各自為政 (Fragmented Decoding)**：
   - 即時綜合監控依賴 `item.Scale`，但清單寫錯為 `0.0001`，算成 `0.21 Hz`。
   - 堵轉測試分頁私自寫了一套階梯式門檻判斷，將 `2060` 誤判為乘以 `0.1`，算成 `206.0 Hz`。
   - 極數辨識模組內部又私自寫了第三套階梯判斷。
   - 缺乏單一核心轉換服務 (Single Source of Truth)，造成同一個暫存器在不同介面各顯神通、數值各異。

### 精確修復方案
1. **確立單一解義轉換服務 (`Dynamometer_KebComm.cs`)**：
   - 新增 `ConvertKebDr05ToFrequency(int rawDr05)` 統一解義 `dr.05`（標準 0.1 Hz 解析 $515 \to 51.5\text{ Hz}$、整數相容）。
   - 保留並完善 `ConvertKebUf00ToFrequency(int rawUf00, int driveId)` 統一解義 `uF.00`（B 載台 $2060 \times 0.025 = 51.50\text{ Hz}$、A 載台 $4120 \times 0.0125 = 51.50\text{ Hz}$）。
2. **即時綜合監控全面升級雙向位址相容與統一解義 (`Dynamometer_KebComm.cs`)**：
   - 在 `DoHmiKebQuery1` 與 `DoHmiKebQuery2` 輪詢中：
     - 若 `item.Address` 為 `0x0400~0x0418`，讀取失敗時自動 fallback 探測 `0x0600~0x0618`；
     - 若 `item.Address` 為 `0x0600~0x0618`，讀取失敗時自動 fallback 探測 `0x0400~0x0418`；
     - 當位址為 `0x0405 / 0x0605` 時，一律呼叫 `ConvertKebDr05ToFrequency` 渲染；
     - 當位址為 `0x0500` 時，一律呼叫 `ConvertKebUf00ToFrequency` 渲染。
3. **等效電路與極數辨識全面收攏 (`Dynamometer_TestEquivCircuit.cs` & `Dynamometer_KebComm.cs`)**：
   - 廢除所有私有階梯門檻計算，全部統一切換為調用 `ConvertKebDr05ToFrequency` 與 `ConvertKebUf00ToFrequency`。
4. **候選清單全面與實機對齊 (`Dynamometer_HMI_WinForms.cs`)**：
   - 參數清單中 dr 群組位址全面更新為實機 `0600` ~ `0605`，兼具向下相容。
5. **版本升級與發布驗證**：
   - 升版至 `2.10.65`。
   - 經 `csc.exe` 編譯通過，執行 `package_release.ps1` 完成 GitHub 雙分支同動推送與 Release 發布。
| V2.105 (beta) | v2.10.63 | 2026-09-14 | 堵轉測試多頻率無縫切換時序、SY.52 轉速映射與低頻電流反向回退機制全面導入：(1)現象與需求：使用者要求堵轉多頻率切換時退回 1/10 額定電壓即可，無需冷卻停頓直接推進下一階段；詢問額定頻率下控制 B 載台 SY.52 的換算方法（以 260V/51.5Hz 為例）；並提出在 1/10 電壓切換轉速後若電流過大的應對防護；(2)致命根因 (Root Cause)：感應馬達鎖死漏抗隨頻率等比例縮小 (Xk ∝ f)，若低頻沿用全額定 1/10 電壓，總阻抗過小引發首步電流暴衝；且舊版換頻留有 1000ms 空轉等待，缺乏動態回退降壓與阻抗繼承；(3)精確修復方案：更新手冊至 V2.1；在 `Dynamometer_TestEquivCircuit.cs` 中實作「阻抗繼承預估法」與「頻率等比例起步電壓折算」，於換頻時先將 uf.09 寫入安全起始電壓再變更 Sy.52/oP.03，握手 400ms 無需冷卻停頓直接啟動；首步加入「反向階梯回退降壓迴圈 (Rollback)」（I > 80% IN 時每次減 2V 直至安全區）；(4)發布與驗證：升版至 2.10.63，經 csc.exe 編譯通過，更新 Firebase 指標並完成雙分支同動推送與 GitHub Release v2.10.63 發布。 |

---

## [V2.106 beta / v2.10.64] - 2026-09-14

### KEB COMBIVERT F5 額定頻率 uF.00 (0x0500) 讀取顯示 0.21Hz 縮放致命缺陷根治

### 現象與佐證
- **使用者回報現象**：使用者在 KEB COMBIVERT F5 變頻器內部設定額定頻率為 `51.5 Hz`，但透過動力計 HMI 介面（包括 KEB 參數即時監控網格與等效電路測試自動採集）讀回顯示的數值卻是嚴重的異常值 `0.21 Hz`。
- **實測數據比對**：
  - 驅動器通訊讀回暫存器原始整數值 (HEX / DEC)：`rawUf00 = 2060` (0x080C)。
  - 舊版 HMI 顯示運算：$2060 \times 0.0001 = 0.206 \to \mathbf{0.21\text{ Hz}}$。

### 致命根因 (Root Cause)
1. **KEB COMBIVERT F5 速度與頻率標準化規格特性**：
   - KEB F5 變頻器內部頻率暫存器（如輸出頻率 `ru.03` 與基頻 `uF.00`）與其設定之最高轉速範圍（Speed Range）緊密綁定：
     - **B 載台（待測端主機，預設 8000 rpm / 400 Hz 範圍）**：速度解析度 $1\text{ unit} = 0.025\text{ Hz}$（即 $1\text{ Hz} = 40\text{ units}$）。
     - **A 載台（加載端負載機，預設 4000 rpm / 200 Hz 範圍）**：速度解析度 $1\text{ unit} = 0.0125\text{ Hz}$（即 $1\text{ Hz} = 80\text{ units}$）。
   - 當使用者設定為 `51.5 Hz` 時，B 載台驅動器暫存器寫入的整數值即為 $51.5 \div 0.025 = \mathbf{2060}$。
2. **舊程式比例因數寫死錯誤**：
   - 在 `Dynamometer_HMI_WinForms.cs` (Line 3129) 的預設參數清單中，誤將 `uF.00` 的 Scale 寫死為 `0.0001`：
     `new { Title = "【uF.00】額定頻率 (0x0500 / 0.0001 Hz)", Scale = "0.0001" }`。
   - 導致 HMI 在解析與格式化時一律執行 $\times 0.0001$，算出 $0.206\text{ Hz}$ 並四捨五入為 `0.21 Hz`！
3. **INI 歷史配置繼承與等效電路階梯判別失效**：
   - 使用者本機的 `dynamometer_layout.ini` 繼承了舊版的 `0.0001` Scale，重啟程式仍載入錯誤縮放。
   - `Dynamometer_TestEquivCircuit.cs` (Line 1568) 內部使用舊有的數值大小分支判斷（`if (r_uf00.Value >= 200) ufFreq = r_uf00.Value * 0.1`），將 2060 誤算為 206 Hz，缺乏對 0.025 / 0.0125 LSB 解析度之雙向適配。

### 精確修復方案
1. **建立專屬頻率轉換核心 (`Dynamometer_KebComm.cs`)**：
   - 新增 `ConvertKebUf00ToFrequency(int rawUf00, int driveId)` 靜態函式：
     - **高解析度模式辨別**：若 $\text{raw} \ge 100000$，採用 $0.0001$ 比例解析。
     - **標準速度解析度模式**：B 載台（Drive 2）採用 $\times 0.025$（$2060 \times 0.025 = 51.50\text{ Hz}$）；A 載台（Drive 1）採用 $\times 0.0125$（$4120 \times 0.0125 = 51.50\text{ Hz}$）。
     - **智能範圍檢驗**：若換算結果落在有效工業頻率區間（$5.0 \sim 400.0\text{ Hz}$），即視為正確解析結果。
   - 在 `DoHmiKebQuery1` 與 `DoHmiKebQuery2` 即時輪詢處理中，若 `item.Address == 0x0500`，強制走專屬轉換，徹底根除網格渲染錯誤。
2. **修正 HMI WinForms 參數候選清單與 INI 防禦清洗 (`Dynamometer_HMI_WinForms.cs`)**：
   - 候選清單 Line 3129：將 Scale 修正為 `"0.025"`，標題更新為 `"【uF.00】額定頻率 (0x0500 / 0.025 Hz)"`。
   - INI 載入防禦清洗（Line 2544 與 Line 2586）：在載入 `KebMonitors1` 與 `KebMonitors2` 時，凡偵測到 `addr == 0x0500` 且其 scale 為 `0.0001` 或 `0.01`，自動校正為對應載台之標準比例（載台 1 為 `0.0125`、載台 2 為 `0.025`），避免使用者因本機舊 INI 設定而持續看到錯誤數值。
3. **等效電路測試參數蒐集同步修復 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 在 `CheckAndCollectDrUfParams()` 讀取 `uF.00` 暫存器時，全面替換舊有錯誤階梯判斷，改為直接呼叫 `ConvertKebUf00ToFrequency(r_uf00.Value, node)`，確保等效電路測試各頻率點試驗與同步檢查所採集到的額定頻率精確為 `51.50 Hz`。
4. **版本發布與發布驗證**：
   - 升版至 `2.10.64`。
   - 遵守 Rule 6 (WinXP .NET 4.0 x86)、Rule 8 (無未經授權開啟瀏覽器) 與 Rule 9 (零 Emoji)。
   - 執行 `package_release.ps1` 通過編譯，完成 GitHub `gh-pages` 與 `master` 雙分支同步推送，更新 Firebase RTDB 版本指標，並發布 GitHub Release v2.10.64。
| V2.104 (beta) | v2.10.62 | 2026-09-14 | 堵轉測試 8 頻率自動化試驗與參數同步檢查全面升級：(1)現象與需求：使用者要求堵轉測試支援 8 頻率點（額定頻率、25%、30%、40%、50%、60%、2倍額定、4倍額定），於額定頻率時蒐集 dr/uf 參數並檢查同步狀態（未同步彈窗警告）；試驗自 1/10 額定電壓開始，以 1V 增幅 5 次估算目標電壓，再以自適應遞減梯度 (最大 5V) 逼近額定電流；達標後連續採樣 30 筆電氣量，剔除電流最高 5 筆與最低 5 筆取中位 20 筆平均；(2)致命根因與實作架構：舊版僅支援單一額定頻率點粗略調壓，缺乏多頻率自動掃描、無極端值剔除演算法、且無 dr/uf 參數雙向同步檢查機制；(3)精確修復方案：在 `Dynamometer_TestEquivCircuit.cs` 中擴充 `LockedSweepItem` 資料結構，支援 8 頻率全自動/單頻測試、背景工作執行緒、自適應調壓、30 筆數據去極端值演算法、8 頻成果對話框與 CSV 匯出；同步相容漏抗頻率折算；落實 Rule 9 全面清除 Emoji；(4)版本發布：升級至 2.10.62，完成編譯打包與 gh-pages / master 雙分支同動推送。 |
| V2.103 (beta) | v2.10.61 | 2026-09-14 | 修復等效電路成果工具列三顆按鈕 Emoji 導致 GDI+ 文字完全消失問題；制定 GEMINI.md Rule 9 禁止 Emoji 鐵律。 |
...
### 堵轉測試多頻率無縫切換時序、SY.52 轉速映射與低頻電流動態回退防護

### 現象與需求
1. **多頻率無縫切換**：使用者指示多頻率試驗時，前頻完成後僅需退回 $1/10$ 額定電壓安全起點即可，無需冷卻停頓，直接接續推進下一頻率測試。
2. **SY.52 控制映射**：使用者詢問由額定頻率（如額定電壓 260V、額定頻率 51.5Hz）換算控制 B 載台速度暫存器 `Sy.52` 的實作方法。
3. **低頻電流過大應對防護**：使用者提出在 1/10 電壓下改變轉速（頻率）後若實測電流過大時的因應策略。
4. **規範與程式同步導入**：將討論之控制流程與應對機制正式寫入說明文件 [`LOCKED_ROTOR_TEST_OPERATION_CONTROL_PROTECTION_MANUAL.md`](file:///c:/Users/peter/OneDrive/Desktop/AI_Projects/Dyanmometer/LOCKED_ROTOR_TEST_OPERATION_CONTROL_PROTECTION_MANUAL.md)，並導入 C# 原始碼。

### 致命根因 (Root Cause)
1. **低頻阻抗急降引發過電流**：感應馬達轉子機械鎖死時，短路阻抗主要由漏抗構成（$Z_k \approx \sqrt{R_k^2 + (2\pi f L_k)^2}$）。當測試頻率降低（如 1/4 頻率 12.875Hz），漏抗 $X_k$ 縮小為 1/4，總阻抗趨近微小之相電阻 $R_k$。若一律沿用全額定之 1/10 電壓（26V），初測電流易衝破額定電流甚至觸發變頻器硬體過電流跳脫。
2. **缺乏動態回退與阻抗繼承**：舊版換頻時有 1000ms 空白等待，且直接開始升壓探測，若初始電流已經偏大仍盲目加壓，將引發熱累積與過流跳脫。

### 精確修復方案
1. **更新堵轉操作手冊至 V2.1 (`LOCKED_ROTOR_TEST_OPERATION_CONTROL_PROTECTION_MANUAL.md`)**：
   - 納入雙頻/多頻無縫切換時序圖（降至 1/10 電壓 $\rightarrow$ 寫入 SY.52 $\rightarrow$ 握手 400ms $\rightarrow$ 零冷卻直接推進）。
   - 明列 SY.52 轉速換算公式 $n_{sync} = 120 \times f_{test} / P$，並提供 260V/51.5Hz 下 2 極、4 極、6 極之轉速設定查表。
   - 建立低頻過電流之四重應對防護策略（頻率等比折算起步電壓、首步反向階梯回退降壓、阻抗繼承預估法、變頻器 uf.01 Boost 檢查）。
2. **導入 C# 原始碼 (`Dynamometer_TestEquivCircuit.cs`)**：
   - **阻抗繼承預估法**：若前一頻率已完成並測得 $L_k$ 與 $R_k$，直接外推新頻率理論目標電壓，取 60% 作為起步電壓。
   - **頻率等比例折算**：若無前頻歷史數據，以 $V_{start} = \text{Clamp}(V_N \times 0.10 \times \text{Ratio}, 5\text{V}, 26\text{V})$ 折算，根除低頻電流偏大。
   - **軟切換防護時序**：先寫入 `uf.09 = startV`，再寫入 `Sy.52` (0x0034) 與 `oP.03` (0x0303)，握手 400ms 立即啟動。
   - **首步電流反向回退機制 (Rollback Loop)**：初測電流 $I_{init} > 0.80 \, I_N$ 時，立即禁止加壓並啟動階梯回退（每次 $-2\text{V}$，間隔 300ms 快速響應），直到退入安全水位（$\le 70\% \, I_N$）再微調探測。
   - **段間零冷卻推進**：採樣完成後立即將 `uf.09` 壓降至下一頻率的折算起步電壓，間隔 300ms 即刻無縫推進下一頻率。
3. **版本升級與發布驗證**：
   - `APP_VERSION` 升級至 `2.10.63`。
   - 經 `csc.exe` (x86 .NET 4.0 WinXP 相容模式) 零 Error 編譯通過。
   - 執行 `package_release.ps1` 完成本機輪替備份、Firebase 雲端版本指標同步、GitHub Release v2.10.63 發布及 `gh-pages` / `master` 雙分支同動推送。

---

## [V2.104 beta / v2.10.62] - 2026-09-14

### 堵轉測試 8 頻率自動化試驗、參數同步檢查與 30 筆採樣去極端值演算法

### 現象與需求
使用者具體規格要求：
1. **(1) 額定頻率 (1.0x)**：`dr` 與 `uf` 參數可蒐集，順便檢查是否有同步，若沒有彈出視窗提醒。
2. **(2) 25% 額定頻率 (0.25x)**
3. **(3) 30% 額定頻率 (0.30x)**
4. **(4) 40% 額定頻率 (0.40x)**
5. **(5) 50% 額定頻率 (0.50x)**
...
7. **(7) 2 倍額定頻率 (2.00x)**
8. **(8) 4 倍額定頻率 (4.00x)**
- **試驗電壓控制流程**：起始電壓從 $1/10$ 額定電壓（`uf09` 可讀取蒐集）開始，做 $1\text{V}$ 增幅 5 次後去估測目標電流所需電壓，再以最大 $5\text{V}$（越接近目標電流縮小增量）一個梯度自適應逼近達到額定電流。
- **數據採樣運算**：達到額定電流後採樣 30 筆電氣量數據，去掉最高與最低電流各 5 筆（共剔除 10 筆極端值），取剩餘 20 筆之算術平均值記錄。

### 致命根因 (Root Cause)
1. 舊版堵轉測試僅支援單一頻率（固定於額定頻率），無法評估馬達在低頻與弱磁高頻下的漏抗飽和特徵。
2. 缺乏對 KEB 驅動器之運轉參數 (`dr`) 與馬達設定參數 (`uf`) 的雙向一致性校驗，若現場人員修改參數而未寫入 EEPROM 同步，易造成堵轉測試基底參數偏差。
3. 舊版單點取樣易受電網諧波、電流感測雜訊波動影響，缺乏高精度多點剔除極端值之統計平均機制。

### 精確修復方案
1. **8 頻率資料結構與常數**：在 `Dynamometer_TestEquivCircuit.cs` 建立 `LockedSamplePoint` 與 `LockedFreqSweepItem`，涵蓋 8 個頻率倍率、各頻率專屬之起始電壓、估測電壓、收斂電壓、30 筆採樣集、實測漏抗與折算額定漏抗。
2. **dr/uf 參數採集與同步檢查**：實作 `CheckAndCollectDrUfParams()`，自動經由 RS-485 讀取驅動器之 `dr.00`~`dr.04`、`dr.24` 與 `uf.00`~`uf.09`，比對額定頻率、電流與電壓一致性；若有不匹配即彈出警告對話框提醒操作者。
3. **自適應調壓演算法**：
   - 變頻器頻率設定：透過 `Sy.52` (0x0034) 與 `oP.03` (0x0303) 給定同步轉速 $N_{sync} = 120 \times f_{test} / P$。
   - 電壓起始：以 $V_{start} = \max(5, \text{Round}(V_N \times 0.1))$ 寫入 `uf09`。
   - 探測階段：以 $1\text{V}$ 增幅 5 次取電流探測點，利用最小平方法估算達額定電流所需目標電壓 $V_{est}$。
   - 漸進逼近：梯度上限 $5\text{V}$，誤差小於 $10\%$ 時逐步縮小為 $2\text{V} \rightarrow 1\text{V}$，直到電流進入 $\pm 2.5\%$ 額定電流區間。
4. **30 筆去極端值平均採樣**：連續採樣 30 筆電氣量（採樣間隔 80ms），依電流值大小排序，剔除最低 5 筆與最高 5 筆，取中間 20 筆之電壓、電流、功率與功率因數算術平均。
5. **UI 控制項與互動擴充**：新增 `[同步檢查]`、`[AI] 單頻測試`、`[>>] 全頻掃描` 與 `[表] 8頻記錄`，並支援一鍵匯出 CSV 報表與套用至等效電路計算。
6. **Rule 9 Emoji 清除**：全自繪與 WinForms 控制項 Text 屬性全面禁用 Emoji，確保 Windows XP GDI+ 穩定渲染。
7. **發布與雙分支同動**：升版至 `2.10.62`，完成打包發布與 `gh-pages` / `master` 同動推送。
...
1. **使用者明確需求**：
   - 「Aa這個功能也可以刪掉了」
2. **實測與版面架構佐證**：
   - 頂部工具列原本配置有「Aa」紫色字體大小客製化彈窗按鈕（`btnFontCustomizer`，寬度 42px，高度 34px）。
   - 在系統全面導入三段式 Dock 排版規範與高解析度標準字級（Consolas 數值 / 微軟正黑體 UI）後，個別字級手動微調已不再使用，且容易因誤觸彈窗更動字級造成版面視覺破壞。刪除此按鈕可精簡頂部操作鏈路，將核心測試工具（通訊測試、儀表微調、診斷LOG、緊急停機）緊湊排列。

---

### 🔍 致命根因分析 (Root Cause)
1. **功能冗餘佔據介面焦點**：
   - 歷史遺留之字體設定彈窗已非動力計即時量測、自動控制或安全保護之核心功能，佔據工具列常駐位置造成視覺干擾。
2. **工具列水平佈局需要無縫遞補**：
   - 刪除 `btnFontCustomizer` 後若未重新計算並調整後續按鈕（設備通訊設定、儀表校正、診斷 LOG）之 X 座標，將造成工具列出現 50px 破洞空白，影響專業工控軟體之 UI 精緻度。

---

### 🛠️ 精確修復方案 (Exact Implementation)
1. **移除字體設定按鈕宣告與實例化 (`Dynamometer_HMI_WinForms.cs`)**：
   - 移除成員變數 `btnFontCustomizer`。
   - 移除 `btnFontCustomizer` 按鈕建立、樣式指派、ToolTip 與點擊彈窗事件監聽。
   - 自 `pnlTop.Controls.AddRange` 中徹底移除此控制項。
2. **緊湊重整頂部工具列水平排版 (`Dynamometer_HMI_WinForms.cs`)**：
   - `btnDeviceSettings` (設備通訊設定工具箱) X 座標調整為 `398`。
   - `btnCalibrationSettings` (儀表校正微調) X 座標調整為 `448`。
   - `btnCrashLogs` (崩潰診斷日誌) X 座標調整為 `498`。
3. **版本同步與發布**：
   - 內部版本號提升至 `v2.10.58`。
   - 執行 `package_release.ps1 -Version 2.5.0` 完成全自動編譯、打包與雙分支同步推送到 `gh-pages` 與 `master`。

---

## [V2.99 beta / v2.10.57] - 2026-09-14
| V2.98 (beta) | v2.10.56 | 2026-09-14 | 堵轉多頻試驗折算、KEB uf09 自適應調壓閉迴路、電流時間與轉速雙重保護閾值及透明化告警：(1)現象與使用者需求：使用者指出堵轉測試有額定頻率、1/2頻率、1/4頻率等多種測法需納入考量；保護機制應以額定電流IN為基準設定110%閥值且持續10秒立即停止；uf09調壓應以類似扭力追隨之自適應閉迴路，先以1V調整5次估測A/V斜率，給予1/2預期電壓後驗算修正，若最小變化量±1V無法重合則依最接近值收斂；堵轉時若轉速有數據且持續超過3秒應判定脫扣跳脫；所有保護作用必須彈窗說明原因杜絕誤會為BUG，並記錄LOG；(2)IEEE Std 112 多頻率漏抗折算引擎：實作 cmbEquivLockedFreq (1.0x / 0.5x / 0.25x)，實測頻率漏抗自動依 Xk(f0) = Xk_meas * (f0/ftest) 線性折算，電感 Lk(mH) 作為固有物理量精確保持；(3)自適應動態調壓閉迴路 (Auto-Tune State Machine)：5步1V探測斜率 k=ΔI/ΔV -> 給予 1/2 預期增量阻尼半幅逼近 (防飽和超調) -> 驗算修正 -> 最小步階比較收斂鎖定，自動採樣鎖定電氣量；(4)雙重保護閾值與透明化告警：tmrLockedWatchdog 實施電流 > 110% IN 持續 10s 即時跳脫、轉速 > 5rpm 持續 3s 治具脫扣跳脫，跳脫時立即 Sy50=0 切斷輸出並將 uf09 強制歸零，彈出詳細數據對話框明確告知保護原因；(5)技術手冊升級至 V2.0：完整更新 LOCKED_ROTOR_TEST_OPERATION_CONTROL_PROTECTION_MANUAL.md。 |
...
1. **使用者明確需求**：
   ...
   - 「B載台 op00=5」
2. **實測與控制需求佐證**：
   - 現場動力計機台日常作業中，上位機 HMI 軟體關閉後，工程師或現場作業人員需要透過電控箱的實體控制面板、旋鈕或端子排對馬達與負載進行手動點動、寸動調校或基本測試。
   - 若程式退出時變頻器仍停留在 HMI 通訊控制模式（例如 `op.01 = 0` 由 Modbus 串列通訊控制運轉方向、`cs.15 = 3` 由上位機總線下發轉矩指令），現場實體控制端子將被變頻器內部邏輯鎖定忽略，導致操作人員無法手動控制機台運轉，甚至造成安全隱患。

---

### 🔍 致命根因分析 (Root Cause)
1. **未定義標準純手動退出狀態**：
   - 舊架構在 `FormClosing` 或 `DisconnectHardware()` 中，僅嘗試將連線初期備份的參數原樣回寫（`RestoreHmiKebInitialParams`）。若該變頻器在啟動 HMI 連線前本身就被設定在非手動狀態，或是備份過程因封包干擾未能建立標記（`HasBackup == false`），則程式關閉時變頻器不會被重設至純手動模式。
2. **退出生命週期看門狗時間裕度不足**：
   - 原先非同步關閉執行緒的 `ExitWatchdog` 設定為 1200ms。當離開程式需連續透過 RS485 向兩台變頻器發送 Sy50=0、cs18=0、op01=7、cs15=1/op00=5 多道寫入指令並等待 Quittung (確認電文) 時，1200ms 可能在通訊稍有重試或延遲時被看門狗提早 Process.Kill()，導致最後的參數未能安全送出。

---

### 🛠️ 精確修復方案 (Exact Implementation)
1. **實作純手動模式切換核心 (`Dynamometer_KebComm.cs`)**：
   - 新增 `public void SwitchToPureManualModeOnExit()`：
     - **安全停機與轉矩卸載**：針對 A/B 載台先發送 `Sy.50 = 0` (STOP) 與 `cs.18 = 0` (轉矩歸零)，確保變頻器在無負載衝擊狀態下切換控制源。
     - **A 載台 (Drive 1)**：寫入 `op.01 = 7` (0x0301，端子控制運轉方向) 與 `cs.15 = 1` (0x0F0F，類比轉矩指令/純手動)。
     - **B 載台 (Drive 2)**：寫入 `op.01 = 7` (0x0301，端子控制運轉方向) 與 `op.00 = 5` (0x0300，過程數據/純手動頻率控制)。
     - 同步於日誌輸出 `KEB_EXIT` 與 `KEB_RESTORE` 紀錄。
   - 強化 `RestoreHmiKebInitialParams(int driveId)`：在回寫原始備份後，強制確保寫入上述純手動模式參數，杜絕無備份時跳過純手動設定的缺漏。
2. **全生命週期退出點全域覆蓋 (`Dynamometer_HMI_WinForms.cs`, `Dynamometer_WebServer.cs`)**：
   - **視窗關閉 (`FormClosing`)**：將 `ExitWatchdog` 超時時間放寬至 2500ms，並在 ThreadPool 非同步關閉流程頂端最優先調用 `SwitchToPureManualModeOnExit()`。
   - **手動中斷連線 (`DisconnectHardware`)**：在關閉 `spKeb1` 與 `spKeb2` 串列通訊埠之前，強制執行 `SwitchToPureManualModeOnExit()`。
   - **線上熱更新重啟 (`ExecuteHotSwapAndRestart`)**：在進行 EXE 替換與 Process.Start 重啟前，先調用 `MainForm.Instance.SwitchToPureManualModeOnExit()`。
3. **版本同步與發布**：
   - 內部版本號提升至 `v2.10.57`。
   - 透過 `package_release.ps1 -Version 2.5.0` 打包編譯，並落實 Rule 7 雙分支推送（`gh-pages` 與 `master`）。
...
### 🎯 現象與佐證 (Verbatim Excerpts & Requirements)
1. **使用者明確需求**：
   - 「堵轉測試有很多種測法，有額定頻率，1/2額定頻率，1/4額定頻率。這也要考慮進去」
   - 「另外保護的部分應該要有時間和電流閥值的限制，閥值參考要從額定電流來做參考，超過10%且持續10秒就需要立即停止。」
   - 「uf09的電壓調整是可以動態調整，但調整的速度也是需要類似扭力追隨一樣，先1V調整5次去估測每A/V的關係，在計算出預期的目標電壓，給予1/2的預期電壓後再度驗算是否如預期，後再修正下次給定數值，另一種可能就是最小變化量+-1V也沒辦法達到目標電流(無法重合)那就依照最接近的數值來決定。」
   - 「另一個保護機制若做堵轉測試時發現扭力計轉速有數據且持續超過3秒，也要跳脫且告知使用者原因。」
   - 「所有保護機制若作用都需要跳出保護的原因，這樣才不會讓使用者誤會是不是軟體的BUG，LOG也要記錄。」
2. **實測與數據流架構佐證**：
   - IEEE Std 112 第 9.2 節規範指出：大中型感應馬達在額定全頻（50/60 Hz）下進行堵轉測試時，轉子導條集膚效應強烈，轉子漏抗會嚴重失真；若採用 1/2 頻率（25/30 Hz）或 1/4 頻率（12.5/15 Hz）測試，能大幅降低深槽集膚畸變，但測得之漏抗必須線性折算回額定基準頻率。
   - 堵轉狀態下轉子無自冷風扇且定轉子電流極大，若未以馬達額定電流 $I_N$ 作為保護基準，長時間承受大電流必導致絕緣漆包線熱崩潰燒機；同時若機械鎖死夾具脫扣滑脫，馬達轉動會造成劇烈衝擊危險。
   - 過去手動調整 `uf.09` 難以拿捏步幅，工程師反覆試湊容易超調或產生過大電流衝擊；且因變頻器電壓以整數 1V 為最小解析度，物理上極可能無法精準剛好重合目標電流小數點，必須具備「最接近值自動抉擇收斂」演算法。

---

### 🔍 致命根因與架構設計 (Root Cause & Architectural Blueprint)
1. **測試頻率缺乏折算機制**：
   - 過去系統假設堵轉測試一律在額定頻率下進行，缺乏頻率選擇與漏抗折算公式 $X_k(f_N) = X_{k,meas} \times (f_N / f_{test})$，無法支援大中型電機規範標準試驗。
2. **缺乏基於額定電流 $I_N$ 的動態時限保護**：
   - 僅依賴變頻器硬體過電流保護（通常設定為 150%~200%），反應過慢且無法防範額定電流 110% 長時間熱累積。
3. **無治具脫扣之轉速異常偵測**：
   - 堵轉應絕對靜止（0 rpm），若治具鬆脫馬達被迫旋轉，舊架構未對轉速持續超過 3 秒進行專屬判定與緊急跳脫。
4. **調壓缺乏閉迴路阻尼演算法**：
   - 感應馬達堵轉阻抗非純線性，若直接全步幅給予預期電壓容易造成超調；必須以「5 步 1V 探測斜率」+「給予 1/2 增量阻尼」+「驗算修正」+「最接近值收斂抉擇」閉環演算。

---

### 🛠️ 精確修復方案 (Exact Resolution & Implementation)
1. **IEEE Std 112 多頻率試驗與漏抗折算引擎 (`Dynamometer_TestEquivCircuit.cs`)**：
   - 實作 `cmbEquivLockedFreq` 下拉選單：
     - `額定頻率 (50/60 Hz) [1.0x]`
     - `1/2 額定頻率 (25/30 Hz) [0.5x, 推薦]`
     - `1/4 額定頻率 (12.5/15 Hz) [0.25x]`
   - 於 `ExecuteEquivCircuitCalculation` 中注入精準折算：實測頻率下之漏抗 $X_{k,meas} = \sqrt{Z_k^2 - R_k^2}$，折算回額定頻率 $f_N$ 漏抗 $X_k(f_N) = X_{k,meas} / \text{Ratio}$，換算漏電感 $L_k(\text{mH}) = \frac{X_{k,meas}}{2\pi f_{test}} \times 1000$ 作為固有物理量恆定保存。
2. **自適應動態調壓閉迴路引擎 (`ToggleAutoTuneUf09` / `RunAutoTuneUf09Step`)**：
   - **起步防護**：啟動前檢查軸轉速，若 $> 5\text{ rpm}$ 立即閉鎖禁止啟動；初始電壓重置為安全起點（12V）。
   - **階段 1 (5 步 1V 斜率探測)**：每 1 秒微調 $+1\text{V}$，連續 5 步記錄 $(V, I)$ 計算局域斜率 $k = \Delta I / \Delta V$ (A/V)。
   - **階段 2 (目標預估與 1/2 阻尼半幅逼近)**：計算預估增量 $\Delta V_{pred} = (I_N - I_{cur}) / k$，**給予 1/2 預期增量**（限幅最大 $\pm 10\text{V}$），徹底防止非線性飽和或超調。
   - **階段 3 (驗算與動態修正)**：取樣實際電流響應，更新局域斜率，持續向容許帶微步逼近。
   - **階段 4 (最小步階比較收斂鎖定)**：若最小變化量 $\pm 1\text{V}$ 無法剛好重合目標電流，自動比較兩側偏差絕對值，**選定與目標電流最接近之電壓值收斂鎖定**，並自動觸發堵轉電氣量採樣！
3. **即時雙重安全防護看門狗 (`TmrLockedWatchdog_Tick` / `TriggerLockedProtectionTrip`)**：
   - **電流閾值超限 10 秒跳脫**：以 $I_N$ 為基準，當實測電流 $> 1.10 \times I_N$ 且持續達 10 秒，看門狗即刻強制觸發緊急跳脫！
   - **轉速異常大於 5 rpm 超過 3 秒跳脫**：偵測到軸轉速 $> 5\text{ rpm}$ 且持續達 3 秒，立即判定機械治具脫扣或鎖死失效，強制跳脫！
   - **主動處置措施**：跳脫瞬間立即下達 `Sy.50 = 0` 切斷變頻器激磁，並將 `uf.09` 輸出電壓降至最低安全值 0V，中止調壓。
4. **透明化告警對話框與全方位日誌記錄 (Zero-Confusion UX)**：
   - 跳脫時彈出完整警示視窗，明確標註保護類型、基準額定電流、保護閾值、實測數值、累計秒數與處置措施，明確說明此為設備主動防護，絕非軟體 BUG；同步以 `LOCKED_PROT_TRIP` 標籤寫入整合日誌。
5. **堵轉測試專用手冊全面升級至 V2.0**：
   - 於 `LOCKED_ROTOR_TEST_OPERATION_CONTROL_PROTECTION_MANUAL.md` 詳細圖解多頻試驗原理、四階段調壓狀態機、雙重閾值保護流程與告警範例。
